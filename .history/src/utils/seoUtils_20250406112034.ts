...
// Define interface for structured data to fix TS errors
interface PlumberStructuredData {
  "@context": string;
  "@type": string;
  "@id": string;
  name: any;
...
export function generatePlumberStructuredData(plumber: any): string {
  const structuredData: PlumberStructuredData = {
  ...
  if (plumber.reviews && plumber.reviews.length > 0) {
    structuredData.review = plumber.reviews.slice(0, 5).map((review: any) => ({
      "@type": "Review",
      author: {
        "@type": "Person",
        name: review.Name,
