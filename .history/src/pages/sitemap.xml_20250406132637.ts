...
export const GET: APIRoute = async ({ site }) => {
  ...
  // Add city pages (with higher limit than before)
  const cityPages = [];
  for (const state of states) {
    const topCities = state.cities
      .sort((a, b) => b.plumberCount - a.plumberCount)
      .slice(0, 50); // Increased from 20 to 50 cities per state
  ...
  // Add plumber pages (with higher limit than before)
  const allPlumbers = await loadPlumberData();
  const topPlumbers = allPlumbers
    .sort((a, b) => (b.reviewCount || 0) - (a.reviewCount || 0))
    .slice(0, 3000); // Increased from 1000 to 3000 plumbers

  const plumberPages = topPlumbers.map((plumber) => ({
